...
def run_live_pipeline(spec: PipelineSpec) -> None:
    """
    Build and run the live camera pipeline.

    Assumptions for the working Hito 7 version:
    - build_pipeline_from_spec(...) returns BuiltPipeline with:
        - pipeline
        - outputs
        - metadata
    - outputs contains output handles such as:
        - preprocessed_frame
        - detections
        - tracklets (optional)
    ...
    for name, output_handle in built.outputs.items():
        try:
            ...
            print(f"[live_runner] Could not create output queue for '{name}': {exc}")
